...
def cb(data):
    print("Distance: {} cm".format(data[2]))

def sonar(board, t_pin, e_pin, cb):
    board.set_pin_mode_sonar(t_pin, e_pin)
    while True:
        detected_faces = []
        try:
            time.sleep(.01)
            ...
            if dist >= 10 and dist <= 50:
                print("Face Detection Zone")
                auth_example()
            else:
                print("Please get close !")
...
def on_image(image):
    try:
        ...
        if not window_init:
            ...
            cv2.resizeWindow(WINDOW_NAME, (int(980 / 1.6), int(1280 / 1.6)))
            window_init = True
...
if __name__ == '__main__':
    ...
    print('Firmata Protocol has been setup ...')
    main()
